...
function oldestDeploy(deploys: Bucket[]) {
  return deploys.reduce((a, b) =>
    moment(a.createdAt) < moment(b.createdAt) ? a : b,
  )
}

function commitsPerDeploysInMonth(
  commits: Bucket[],
  deploys: Bucket[],
  month: string,
) {
...
function releaseCandidatesPerDeploys(
  commits: Bucket[],
  deploys: Bucket[],
  endDate: string,
) {
  ...
  while (build(currentDate) < build(endDate)) {
    candidatesPerDeploys.push([
      currentDate,
      commitsPerDeploysInMonth(commits, deploys, currentDate),
    ])
...
export { exportGraphData, releaseCandidatesPerDeploys }
